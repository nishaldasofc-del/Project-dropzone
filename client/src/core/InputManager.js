...
export class InputManager {
    ...
    setupKeyboard() {
        document.addEventListener('keydown', (e) => {
            ...
            switch (e.code) { 
                ...
                case 'Tab': e.preventDefault(); this.fireEvent('inventory'); break; 
    ...
    setupTouch() {
        ...
        const handleTouchStart = (e) => {
            // This prevents zoom/scroll in the game, but allows typing on inputs and clicking buttons!
        ...
        const handleTouchMove = (e) => {
            // Keep interactive elements uncluttered during movement
            const isInteractive = ['INPUT', 'TEXTAREA', 'SELECT'].includes(e.target.tagName);
            if (!isInteractive) {
                e.preventDefault();
            }
    ...
    setupActionButtons() {
        ...
        // Jump button - short trigger
        const jumpBtn = document.getElementById('btn-jump');
        if (jumpBtn) {
            const triggerJump = (e) => {
                e.stopPropagation();
                this.actions.jump = true;
                setTimeout(() => { this.actions.jump = false; }, 150);
            };
            jumpBtn.addEventListener('touchstart', triggerJump, { passive: true });
